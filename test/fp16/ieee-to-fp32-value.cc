...
TEST(IEEE_FP16_VALUE, normalized_powers_of_2) {
	...
	const float min_po2_value = fp16b_to_fp32_ieee(min_po2_f16);
	...
	const float eighths_value = fp16b_to_fp32_ieee(eighths_f16);
	...
	const float quarter_value = fp16b_to_fp32_ieee(quarter_f16);
	...
	const float half_value = fp16b_to_fp32_ieee(half_f16);
	uint32_t half_bits;
	...
	const float one_value = fp16b_to_fp32_ieee(one_f16);
	uint32_t one_bits;
	...
	const float two_value = fp16b_to_fp32_ieee(two_f16);
	uint32_t two_bits;
	...
	const float four_value = fp16b_to_fp32_ieee(four_f16);
	uint32_t four_bits;
	...
	const float eight_value = fp16b_to_fp32_ieee(eight_f16);
	uint32_t eight_bits;
	...
	const float sixteen_value = fp16b_to_fp32_ieee(sixteen_f16);
	...
	const float thirtytwo_value = fp16b_to_fp32_ieee(thirtytwo_f16);
	...
	const float sixtyfour_value = fp16b_to_fp32_ieee(sixtyfour_f16);
	...
	const float max_po2_value = fp16b_to_fp32_ieee(max_po2_f16);
...
TEST(IEEE_FP16_VALUE, denormalized_powers_of_2) {
	...
	const float exp2_minus_15_value = fp16b_to_fp32_ieee(exp2_minus_15_f16);
	...
	const float exp2_minus_16_value = fp16b_to_fp32_ieee(exp2_minus_16_f16);
	...
	const float exp2_minus_17_value = fp16b_to_fp32_ieee(exp2_minus_17_f16);
	...
	const float exp2_minus_18_value = fp16b_to_fp32_ieee(exp2_minus_18_f16);
	...
	const float exp2_minus_19_value = fp16b_to_fp32_ieee(exp2_minus_19_f16);
	...
	const float exp2_minus_20_value = fp16b_to_fp32_ieee(exp2_minus_20_f16);
	...
	const float exp2_minus_21_value = fp16b_to_fp32_ieee(exp2_minus_21_f16);
	...
	const float exp2_minus_22_value = fp16b_to_fp32_ieee(exp2_minus_22_f16);
	...
	const float exp2_minus_23_value = fp16b_to_fp32_ieee(exp2_minus_23_f16);
	...
	const float exp2_minus_24_value = fp16b_to_fp32_ieee(exp2_minus_24_f16);
...
TEST(IEEE_FP16_VALUE, zero) {
	...
	const float positive_zero_value = fp16b_to_fp32_ieee(positive_zero_f16);
	...
	const float negative_zero_value = fp16b_to_fp32_ieee(negative_zero_f16);
...
TEST(IEEE_FP16_VALUE, infinity) {
	...
	const float positive_infinity_value = fp16b_to_fp32_ieee(positive_infinity_f16);
	...
	const float negative_infinity_value = fp16b_to_fp32_ieee(negative_infinity_f16);
...
TEST(IEEE_FP16_VALUE, positive_normalized_values) {
	...
	for (int32_t e = -14; e <= 15; e++) {
		for (uint16_t h = 0; h < 0x0400; h++) {
			...
			const float value = fp16b_to_fp32_ieee(fp16);
			uint32_t bits;
...
TEST(IEEE_FP16_VALUE, positive_denormalized_values) {
	for (uint16_t h = 0; h < 0x0400; h++) {
		const float value = fp16b_to_fp32_ieee(h);
		uint32_t bits;
		memcpy(&bits, &value, sizeof(bits));
