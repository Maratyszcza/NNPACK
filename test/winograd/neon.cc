...
TEST(F6x6_3x3, input) {
	...
	tester.testInputTransform2D(nnp_iwt8x8_3x3_with_offset__neon, inputTransformMatrix);
}
...
TEST(F6x6_3x3, kernel) {
	...
	tester.testKernelTransform2D(nnp_kwt8x8_3x3__neon, kernelTransformMatrix);
}
...
TEST(F6x6_3x3, output) {
	...
	WinogradTransformTester()
		...
		.simdWidth(4)
		.errorLimit(1.0e-6f)
		.testOutputTransform2D(nnp_owt8x8_3x3__neon, outputTransformMatrix);
}
