...
TEST(FT8x8, small_batch) {
	...
	for (size_t batchSize = 2; batchSize <= 5; batchSize++) {
		tester.batchSize(batchSize).testOutput(nnp_convolution_algorithm_ft8x8, nnp_activation_relu);
	}
}
...
TEST(FT16x16, small_batch) {
	...
	tester.inputSize(16, 16)
		...
		.errorLimit(1.0e-5);
	for (size_t batchSize = 2; batchSize <= 5; batchSize++) {
		tester.batchSize(batchSize).testOutput(nnp_convolution_algorithm_ft16x16, nnp_activation_relu);
	}
}
...
TEST(WT8x8, small_batch) {
	...
	tester.inputSize(8, 8)
		...
		.errorLimit(1.0e-3);
	for (size_t batchSize = 2; batchSize <= 5; batchSize++) {
		tester.batchSize(batchSize).testOutput(nnp_convolution_algorithm_wt8x8, nnp_activation_relu);
	}
}
...
TEST(FT8x8, few_input_channels) {
	...
	for (size_t inputChannels = 2; inputChannels <= 5; inputChannels++) {
		tester.inputChannels(inputChannels).testOutput(nnp_convolution_algorithm_ft8x8, nnp_activation_relu);
	}
}
...
TEST(FT16x16, few_input_channels) {
	...
	tester.inputSize(16, 16)
		...
		.errorLimit(1.0e-5);
	for (size_t inputChannels = 2; inputChannels <= 5; inputChannels++) {
		tester.inputChannels(inputChannels).testOutput(nnp_convolution_algorithm_ft16x16, nnp_activation_relu);
	}
}
...
TEST(WT8x8, few_input_channels) {
	...
	tester.inputSize(8, 8)
		...
		.errorLimit(1.0e-3);
	for (size_t inputChannels = 2; inputChannels <= 5; inputChannels++) {
		tester.inputChannels(inputChannels).testOutput(nnp_convolution_algorithm_wt8x8, nnp_activation_relu);
	}
}
...
TEST(FT8x8, few_output_channels) {
	...
	for (size_t outputChannels = 2; outputChannels <= 5; outputChannels++) {
		tester.outputChannels(outputChannels).testOutput(nnp_convolution_algorithm_ft8x8, nnp_activation_relu);
	}
}
...
TEST(FT16x16, few_output_channels) {
	...
	tester.inputSize(16, 16)
		...
		.errorLimit(1.0e-5);
	for (size_t outputChannels = 2; outputChannels <= 5; outputChannels++) {
		tester.outputChannels(outputChannels).testOutput(nnp_convolution_algorithm_ft16x16, nnp_activation_relu);
	}
}
...
TEST(WT8x8, few_output_channels) {
	...
	tester.inputSize(8, 8)
		...
		.errorLimit(1.0e-3);
	for (size_t outputChannels = 2; outputChannels <= 5; outputChannels++) {
		tester.outputChannels(outputChannels).testOutput(nnp_convolution_algorithm_wt8x8, nnp_activation_relu);
	}
}
