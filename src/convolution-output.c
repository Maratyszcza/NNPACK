...
struct NNP_CACHE_ALIGN input_transform_context {
	...
	const size_t tiles_block_size;
	const struct fxdiv_divisor_size_t tiles_x_count;
	const struct fxdiv_divisor_size_t tiles_y_count;
	...
	size_t input_channels_block_range,  size_t tiles_subblock_size)
{
	...
	const size_t tiles_block_start                  = context->tiles_block_start;
	const size_t tiles_block_size                   = context->tiles_block_size;
	const struct fxdiv_divisor_size_t tiles_x_count = context->tiles_x_count;
	const struct fxdiv_divisor_size_t tiles_y_count = context->tiles_y_count;
	const size_t input_channels                     = context->input_channels;
	...
	const float (*input)[input_channels][input_size.height][input_size.width] =
		(const float(*)[input_channels][input_size.height][input_size.width]) context->input;
	...
	for (size_t tiles_subblock_offset = 0; tiles_subblock_offset < tiles_subblock_size; tiles_subblock_offset += 1) {
		...
		const struct fxdiv_result_size_t batch_tile_xy = fxdiv_divide_size_t(tile, tiles_x_count);
		const struct fxdiv_result_size_t batch_tile_y = fxdiv_divide_size_t(batch_tile_xy.quotient, tiles_y_count);
		const size_t tile_x = batch_tile_xy.remainder;
		const size_t tile_y = batch_tile_y.remainder;
		const size_t sample = batch_tile_y.quotient;
		...
		transform_function(
			&input[sample][input_channel][input_y][input_x],
			...
			input_size.width,
			input_channels_block_size * tiles_block_size * tuple_elements * sizeof(float),
...
struct NNP_CACHE_ALIGN output_transform_context {
	...
	struct fxdiv_divisor_size_t tiles_y_count;
	...
	size_t output_channels_subblock_size,  size_t tiles_subblock_size)
{
	...
	float (*output)[output_channels][output_size.height][output_size.width] =
		(float(*)[output_channels][output_size.height][output_size.width]) context->output;
	...
	for (size_t tiles_subblock_offset = 0; tiles_subblock_offset < tiles_subblock_size; tiles_subblock_offset += 1) {
		...
		for (size_t output_channels_subblock_offset = 0; output_channels_subblock_offset < output_channels_subblock_size; output_channels_subblock_offset += 1) {
			...
			transform_function(
				...
				&output[sample][output_channel][output_y][output_x],
				&bias[output_channel],
	...
	size_t output_channels_subblock_size)
{
	...
	const size_t tiles_block_start            = context->tiles_block_start;
	size_t tiles_block_size                   = context->tiles_block_size;
	const size_t input_channels_block_size    = context->input_channels_block_size;
	const size_t input_channels_block_start   = context->input_channels_block_start;
	const size_t output_channels              = context->output_channels;
	...
	while (tiles_block_size != 0) {
		...
		input_transform  += tiles_subblock_size * input_channels_block_size * tuple_elements;
		output_transform += tiles_subblock_size * output_channels_subblock_size * tuple_elements;
	}
}
	...
	struct nnp_profile* profile)
{
	...
	const size_t tiles_count = batch_size * tiles_x_count * tiles_y_count;
	...
	const size_t input_transform_size = min(tiles_count, tiles_block_max) * min(input_channels, input_channels_block_max) * transform_tile_size;
	...
	for (size_t input_channels_block_start = 0; input_channels_block_start < input_channels; input_channels_block_start += input_channels_block_max) {
		...
		for (size_t tiles_block_start = 0; tiles_block_start < tiles_count; tiles_block_start += tiles_block_max) {
			...
			struct input_transform_context input_transform_context = {
				...
				.tiles_block_start = tiles_block_start,
				.tiles_block_size = tiles_block_size,
				.tiles_x_count = fxdiv_init_size_t(tiles_x_count),
				.tiles_y_count = fxdiv_init_size_t(tiles_y_count),
				.input_channels = input_channels,
			...
			pthreadpool_compute_2d_tiled(threadpool,
				...
				input_channels_block_size, tiles_block_size,
			...
			for (size_t tuple_index = 0; tuple_index < tuple_count; tuple_index += 1) {
				...
				for (size_t output_channels_block_start = 0; output_channels_block_start < output_channels; output_channels_block_start += output_channels_block_max) {
					...
					struct tuple_multiplication_context tuple_multiplication_context = {
						...
						.input_transform = input_transform +
							tuple_index * tiles_block_size * input_channels_block_size * tuple_elements,
						.kernel_transform = kernel_transform +
					...
					pthreadpool_compute_1d_tiled(threadpool,
						(pthreadpool_function_1d_tiled_t) compute_tuple_multiplication,
	...
	switch (algorithm) {
		...
		case nnp_convolution_algorithm_ft16x16:
			...
			status = compute_fast_convolution_inference(
				...
				input, kernel, bias, output, /*workspace_buffer, workspace_size,*/ NULL, NULL,
