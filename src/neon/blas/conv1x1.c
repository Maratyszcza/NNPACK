	...
	float* output)
{
	...
	float* output3 = output2 + image_size;
	while (image_size >= 4) {
		float32x4_t vinput0, vinput1;
		vinput0 = vinput1 = vld1q_f32(input0);
		input0 += 4;
		if (input_channels_subblock_size > 1) {
			vinput1 = vld1q_f32(input1);
			input1 += 4;
		}
	...
	if (image_size != 0) {
		...
		float32x4_t vinput0, vinput1;
		vinput0 = vinput1 = vreinterpretq_f32_s32(
			vandq_s32(vmask, vreinterpretq_s32_f32(vld1q_f32(&input0[image_size - 4]))));
		if (input_channels_subblock_size > 1) {
			vinput1 = vreinterpretq_f32_s32(
				vandq_s32(vmask, vreinterpretq_s32_f32(vld1q_f32(&input1[image_size - 4]))));
		}
