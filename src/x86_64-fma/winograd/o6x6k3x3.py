...
if __name__ == "__main__":
    ...
    arg_d = Argument(ptr(const_float_))
    arg_wd = Argument(ptr(float_))
    with Function("input_winograd", (arg_d, arg_wd)) as input_winograd_asm:
        reg_d = GeneralPurposeRegister64()
        LOAD.ARGUMENT(reg_d, arg_d)

        reg_wd = GeneralPurposeRegister64()
        LOAD.ARGUMENT(reg_wd, arg_wd)

        ymm_data = [YMMRegister() for _ in range(8)]
        for i, ymm in enumerate(ymm_data):
            VMOVSS(ymm.as_xmm, [reg_d + i * float_.size])

        ymm_data = input_transform(ymm_data)

        for i, ymm in enumerate(ymm_data):
            VMOVSS([reg_wd + i * float_.size], ymm.as_xmm)

        RETURN()

    input_winograd = input_winograd_asm.finalize(abi.detect()).encode().load()

    d = numpy.random.random(8).astype(numpy.float32)
    wd = numpy.empty(8, numpy.float32)

    input_winograd(
        d.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
        wd.ctypes.data_as(ctypes.POINTER(ctypes.c_float)))

    # print(tuple(float(n) for n in wd))
    # print(input_transform_3x3_6x6(*list(d)))

    arg_g = Argument(ptr(const_float_))
    arg_wg = Argument(ptr(float_))
    with Function("kernel_winograd", (arg_g, arg_wg)) as kernel_winograd_asm:
        reg_g = GeneralPurposeRegister64()
        LOAD.ARGUMENT(reg_g, arg_g)

        reg_wg = GeneralPurposeRegister64()
        LOAD.ARGUMENT(reg_wg, arg_wg)

        ymm_g = [YMMRegister() for _ in range(3)]
        for i, ymm in enumerate(ymm_g):
            VMOVSS(ymm.as_xmm, [reg_g + i * float_.size])

        ymm_wg = kernel_transform(ymm_g)

        for i, ymm in enumerate(ymm_wg):
            VMOVSS([reg_wg + i * float_.size], ymm.as_xmm)

        RETURN()

    kernel_winograd = kernel_winograd_asm.finalize(abi.detect()).encode().load()

    g = numpy.random.random(3).astype(numpy.float32)
    wg = numpy.empty(8, numpy.float32)

    kernel_winograd(
        g.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
        wg.ctypes.data_as(ctypes.POINTER(ctypes.c_float)))

    wg_ref = numpy.array(kernel_transform_3x3_6x6(*list(g)))
    assert numpy.max(numpy.abs(wg_ref - wg)) < 1.0e-5
    # print(tuple(float(n) for n in wg))
    # print(kernel_transform_3x3_6x6(*list(g)))

    arg_m = Argument(ptr(const_float_))
    arg_s = Argument(ptr(float_))
    with Function("output_winograd", (arg_m, arg_s)) as output_winograd_asm:
        reg_m = GeneralPurposeRegister64()
        LOAD.ARGUMENT(reg_m, arg_m)

        reg_s = GeneralPurposeRegister64()
        LOAD.ARGUMENT(reg_s, arg_s)

        ymm_m = [YMMRegister() for _ in range(8)]
        for i, ymm in enumerate(ymm_m):
            VMOVSS(ymm.as_xmm, [reg_m + i * float_.size])

        ymm_s = output_transform(ymm_m)

        for i, ymm in enumerate(ymm_s):
            VMOVSS([reg_s + i * float_.size], ymm.as_xmm)

        RETURN()

    output_winograd = output_winograd_asm.finalize(abi.detect()).encode().load()

    m = numpy.random.random(8).astype(numpy.float32)
    s = numpy.empty(6, numpy.float32)

    output_winograd(
        m.ctypes.data_as(ctypes.POINTER(ctypes.c_float)),
        s.ctypes.data_as(ctypes.POINTER(ctypes.c_float)))

    s_ref = numpy.array(product_transform_3x3_6x6(*list(m)))
    assert numpy.max(numpy.abs(s_ref - s)) < 1.0e-5
